...
RegisterNetEvent('crp-interior:client:SetNewState')
AddEventHandler('crp-interior:client:SetNewState', function(bool)
	IsNew = bool
end)
...
function CreateApartmentFurnished(spawn)
	...
	if IsNew then
		SetTimeout(750, function()
			TriggerEvent('crp-clothes:client:CreateFirstCharacter')
			IsNew = false
		end)
	end
    return { objects, POIOffsets }
