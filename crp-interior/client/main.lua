...
function CreateWarehouseShell(spawn)
	local objects = {}

    local POIOffsets = {}
	POIOffsets.exit = json.decode('{"z":1.5,"y":0.1,"x":8.4,"h":102.78}')
	DoScreenFadeOut(500)
    while not IsScreenFadedOut() do
        Citizen.Wait(10)
    end
...
--[[
function CreateWarehouseShell2(spawn)
	local objects = {}

    local POIOffsets = {}
	POIOffsets.exit = json.decode('{"z":1.5,"y":0.1,"x":8.4,"h":358.133972168}')
	DoScreenFadeOut(500)
    while not IsScreenFadedOut() do
        Citizen.Wait(10)
    end
...
function CreateWarehouseShell3(spawn)
	local objects = {}
...
--]]
function CreateWarehouseShell4(spawn)
	local objects = {}

    local POIOffsets = {}
	POIOffsets.exit = json.decode('{"z":1.4,"y":-12.9,"x":5.6,"h":358.133972168}')
	DoScreenFadeOut(500)
    while not IsScreenFadedOut() do
        Citizen.Wait(10)
    end
...
--[[
function CreateWarehouseShell5(spawn)
	local objects = {}
